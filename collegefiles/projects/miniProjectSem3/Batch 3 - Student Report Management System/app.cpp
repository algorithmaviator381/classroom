...
public:
  Student(string n, string roll) : name(n), rollNumber(roll) {}
...
public:
  ...
  void inputMarks() {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      cout << "   -Subject Total: "
           << inSemesterMarks[i] + practicalExamMarks[i] + endSemesterMarks[i]
           << endl;
    }
  }
...
public:
  ...
  float calculatePointer() override { return calculatePercentage() / 9.5; }
  ...
  void displayResult() {
    system("cls");
    cout << "\n\nMarks inputed sucessfully\n" << endl;
    cout << "\nPress any key to continue further to display the result" << endl;
    ...
    for (int i = 0; i < 5; i++) {
      cout << "Subject " << (i + 1) << ":\n";
      cout << "  - In-Semester Marks    : " << inSemesterMarks[i] << "/20\n";
      cout << "  - Practical Marks      : " << practicalExamMarks[i] << "/20\n";
      cout << "  - End-Semester Marks   : " << endSemesterMarks[i] << "/60\n";
      cout << "  - Subject Total Marks  : "
           << inSemesterMarks[i] + practicalExamMarks[i] + endSemesterMarks[i]
           << "/100\n";
      cout << "----------------------------------------------------" << endl;
    }

    cout << "Total Percentage: " << calculatePercentage() << "%" << endl;
...
int main() {
  ...
  getline(cin, name);
  cout << "Enter roll number (E-68 format): ";
  cin >> roll;
